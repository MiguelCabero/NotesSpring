...
class NotePersistApplicationTests {
	...
	void updateNote_WhenCalled_ReturnsNote() {
		...
		final Note noteTwo = new Note(1, "Updated", "textUpdated",
				local, local2, "linkUpdated", "mentionsUpdated", 'A');

		assertEquals(noteTwo,
				noteController.updateNote(note, noteTwo));
	}
	...
	void checkLink_WhenCalled_ReturnsLinkType() {
		final Note note = new Note(1, "description", "text", local,
				local2, "hola@quetal.com", "mentions", 'p');

		assertEquals("It's an email",
				noteController.checkLink(note.getLink()));
	}
	...
	void checkMetion_WhenCalled_ReturnsNameOfSociaMedia() {
		final Note note = new Note(1, "description", "text", local,
				local2, "hola@quetal.com", "@AGLeon29", 'p');

		assertEquals("https://www.twitter.com/AGLeon29",
				noteController.getProfileLink(note));
	}
}
