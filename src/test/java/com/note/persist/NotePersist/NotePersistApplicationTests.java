...
class NotePersistApplicationTests {
	...
	// Test 6
	@Test
	void importNoteFromFile_WhenCalled_ReturnsFile() throws ClassNotFoundException, IOException {
		final Note note = new Note(1, "description", "text", local, local2, "link", "mentions", 'p');
		final String username = System.getProperty("user.name");
		final String path = "C:\\Users\\" + username + "\\Documents\\note1.ser";
	...
	void deleteNote_WhenCalled_ReturnsTrue() {
		...
		final boolean delete = noteController.deleteNote(1);

		Assert.isTrue(delete, "Note removed");
	}

	// Test 8
	@Test
	void deleteNote_WhenCalled_LengthisLess() {
		for (int i = 0; i < 5; i++) {
			final Note note = new Note(i + 1, "description", "text", local, local2, "link", "mentions", 'p');
			noteController.createNote(note);
			noteController.saveNote(note);
		...
		assertEquals(4, noteController.getNotes().size());
	}

	// Test 9
	@Test
	void updateNote_WhenCalled_ReturnsNote() {
		final Note note = new Note(1, "description", "text", local, local2, "link", "mentions", 'p');
		noteController.createNote(note);
		noteController.saveNote(note);

		final Note noteTwo = new Note(1, "Updated", "textUpdated", local, local2, "linkUpdated", "mentionsUpdated",
				'A');

		assertEquals(noteTwo, noteController.updateNote(note, noteTwo));
	}

	// Test 10
	@Test
	void checkLink_WhenCalled_ReturnsLinkType() {
		final Note note = new Note(1, "description", "text", local, local2, "hola@quetal.com", "mentions", 'p');

		assertEquals("It's an email", noteController.checkLink(note.getLink()));
	}

	// Test 11
	@Test
	void checkMetion_WhenCalled_ReturnsNameOfSociaMedia() {
		final Note note = new Note(1, "description", "text", local, local2, "hola@quetal.com",
				"https://twitter.com/alo_oficial?ref_src=twsrc%5Egoogle%7Ctwcamp%5Eserp%7Ctwgr%5Eauthor", 'p');

		assertEquals("twitter", noteController.checkMention(note.getMentions()));
	}
}
